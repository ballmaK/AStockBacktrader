...
class BambooVolume(bt.Strategy):
    """
    主策略程序
    """
    alias = ('Bamboo_Volume',)
    ...
    def __init__(self):
        ...
        for data in self.datas:
            self.lvs[data._name] = btind.bvolume.STDV(data=data, period=self.p.period, br1=self.p.br1, br2=self.p.br2, sr=self.p.sr)
    ...
    def next(self):
        ...
        for i, d in enumerate(self.datas):
            ...
            if not self.broker.getposition(d):  # 没有持仓
                ...
                if self.lvs[d._name].sell[0] > 0: #or stop_less or take_profit:
                    # 执行卖出
