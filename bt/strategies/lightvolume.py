...
def main(code="sh600070", start_cash=1000000, start_date='20210101', end_date='20220101', stake=10000, commission_fee=0.001):
    cerebro = bt.Cerebro()  # 创建主控制器
    # cerebro.optstrategy(PolicyBT, maperiod=240, rate_high=[ i/100 for i in range(80, 100, 2)], rate_low=[ i/100 for i in range(0, 10, 1)])  # 导入策略参数寻优
    # cerebro.optstrategy(PolicyBT, maperiod=90, rate_buy=0.9, rate_sell=[ i/100 for i in range(50, 100, 1)]) 
    # cerebro.optstrategy(PolicyBT, maperiod=(90, 120, 180, 240), rate_high=0.9, rate_low=0.08) 

    # 利用 AKShare 获取股票的后复权数据，这里只获取前 6 列
    # stock_hfq_df = ak.stock_zh_a_hist(symbol='600070', adjust="hfq", start_date='20000101', end_date='20210617').iloc[:, :6]
    # print(stock_hfq_df)
    stock_qfq_df = common.select_stock_daily(code=code, start_date=start_date, end_date=end_date).iloc[:, :6]
    # print(stock_qfq_df)
    ...
    stock_qfq_df['open'] = pd.to_numeric(stock_qfq_df['open'])
    stock_qfq_df['close'] = pd.to_numeric(stock_qfq_df['close'])
    stock_qfq_df['high'] = pd.to_numeric(stock_qfq_df['high'])
    stock_qfq_df['low'] = pd.to_numeric(stock_qfq_df['low'])
    stock_qfq_df['volume'] = pd.to_numeric(stock_qfq_df['volume'])
    stock_qfq_df.index = pd.to_datetime(stock_qfq_df['date'])
    start_date = datetime.strptime(start_date, "%Y%m%d")  # 回测开始时间
    end_date = datetime.strptime(end_date, "%Y%m%d")  # 回测结束时间
    data = bt.feeds.PandasData(dataname=stock_qfq_df, fromdate=start_date, todate=end_date)  # 规范化数据格式
    cerebro.adddata(data)  # 将数据加载至回测系统
    cerebro.addstrategy(LightVolume)  # 将交易策略加载到回测系统中
    ...
    # cerebro.addanalyzer(bt.analyzers.TradeAnalyzer, _name='mysharpe')
    cerebro.addanalyzer(bt.analyzers.SharpeRatio)
    cerebro.addanalyzer(bt.analyzers.Returns)
    # cerebro.addwriter(bt.WriterFile, csv='~/Desktop/backtrader_results/backtrader.c-sv', rounding=2)
    # print("[%s]期初总资金: %.2f" % (code,cerebro.broker.getvalue()))
    results = cerebro.run()  # 用单核 CPU 做优化
    # print("[%s]期末总资金: %.2f" % (code, cerebro.broker.getvalue()))
    ...
    # print(thestrats[0].analyzers.mysharpe.get_analysis())
    # for thestrat in thestrats:
    #     print('夏普比率:', thestrat[0].analyzers.mysharpe.get_analysis()) 
        # pass
        
    # cerebro.plot(style='candle')
    # cerebro.broker.getwriter().save(path='backtest_data.csv')
    
    strats = [ strat for i, strat in enumerate(results)]
    # print(dir(strats[0]))
    ...
    if len(strats[0].orders) > 1:
        ...
        if last_order_date == today and last_order_type == 'BUY':
            return (code, sharpe, rnorm100)
    else:
        print(f"{code},{sharpe},{rnorm100},- no order")


if __name__ == '__main__':
    main(code="sh600070", start_cash=1000000, stake=10000, commission_fee=0.001)